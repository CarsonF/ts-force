...
export const getCalendarDate = (
  ...
  if (d instanceof Date) {
    ...
    throw new Error(
      'Not a valid object.  Must have year(), month(), date() functions'
    );
  } else {
    const parts = d.split('-').map(p => parseInt(p, 10));
    if (parts.length !== 3 || parts.find(p => isNaN(p))) {
      throw new Error(
        'Not a valid CalendarDate string.  Required Format: yyyy-mm-dd'
      );
    }
...
function isMomentish(arg: any): arg is Momentish {
  return (
    arg.year !== undefined && arg.month !== undefined && arg.date !== undefined
  );
}
