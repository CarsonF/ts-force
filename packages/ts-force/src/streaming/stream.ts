...
export class Streaming {
  private readonly listener: CometD;
  private readonly subscribers: Map<string, SubscriptionHandle>;
  ...
  constructor(client?: Rest) {
    ...
    this.listener.configure({
      url: `${client.config.instanceUrl}/cometd/${client.config.version.toFixed(
        1
      )}/`,
   ...
   */
  unregisterTransport(
    transport: 'websocket' | 'long-polling' | 'callback-polling'
  ) {
  ...
  _subscribe(
    ...
    return new Promise((resolve, reject) => {
      ...
      const subscriber = this.listener.subscribe(channel, onEvent, m => {
        if (m.successful) {
  ...
  subscribeToEvent<T>(
    event: string,
    onEvent: (m: PlatformEvent<T>) => void
  ): Promise<SubscriptionHandle> {
   ...
   * @memberof Streaming
   */
  subscribeToTopic<T extends TopicSObject>(
    topic: string,
    onEvent: (m: TopicMessage<T>) => void
  ): Promise<SubscriptionHandle> {
...
// Push Topic
export interface SObjectTopicMessage<T extends RestObject>
  extends Omit<TopicMessage<any>, 'data'> {
  data: {
    event: Event;
    sObject: T;
  };
