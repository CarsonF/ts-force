...
export const generateSelect = (
  fields: Array<string | SFieldProperties>,
  relationships?: (string | SFieldProperties) | Array<string | SFieldProperties>
): string => {
  ...
  if (relationships) {
    ...
    if (relationships instanceof Array) {
      relationshipName = relationships
        .map(f => (typeof f === 'string' ? f : f.apiName))
        .join('.');
    } else {
      relationshipName =
 ...
 */
export const generateInValues = <T>(
  objs: T[],
  valueSelector?: (obj: T) => string
) => {
  let values: any[];
  if (valueSelector) {
