...
export class CompositeCollection {
  ...
  insert = async (
    ...
    const saveResults = (await this.client.request.post(this.endpoint, payload))
      .data;
  ...
  delete = async (
    ...
    return (
      await this.client.request.delete(
        `${this.endpoint}?ids=${sobs
          .map(s => s.id)
          .join(',')}&allOrNone=${allOrNothing !== false}`
      )
    ).data;
  };

  private readonly resetModified = (
    sobs: RestObject[],
    results: SaveResult[]
  ) => {
    // clear out modified
