...
import { composeConditionalClause, ConditionalClause } from './conditional';
import { FieldProps, SFieldProperties, SObject } from '..';
...
export function buildQuery<T>(
  ...
  const fields = new FieldResolver(from);
  const qry = buildQuery(fields);
...
export function composeSOQLQuery(qry: SOQLQuery): string {
  ...
  if (orderBy) {
    ...
    const orderings = orderByClause.map(
      o =>
