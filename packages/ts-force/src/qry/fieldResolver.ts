...
export class FieldResolver<T> {
  ...
  select(
    ...
    const relations = this.traversed.map(r => r.apiName);
    ...
    const fields = fieldArr.map(field => {
   ...
   */
  parent<K extends ParentReferencePropNames<T>>(relation: K) {
    const rel1Meta = this._obj.FIELDS[relation as string];
    const rel1 = (rel1Meta.reference() as any) as SObjectStatic<T[K]>;
   ...
   */
  subQuery<
    K extends RelationPropNames<T>,
    T2 extends T[K] extends Array<infer U> ? U : never
  >(
    relationship: K,
    func: (fields: FieldResolver<T2>) => SOQLQueryParams
  ): string {
    const relationMeta = this._obj.FIELDS[relationship as string];
    const fields = new FieldResolver(
      (relationMeta.reference() as any) as SObjectStatic<T2>
    );
    const subQuery = func(fields);
    return `(${composeSOQLQuery({
      ...subQuery,
      ...{ from: relationMeta.apiName },
    })})`;
  }
}
