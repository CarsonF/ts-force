...
import { getCalendarDate } from '../../src/utils/calendarDate';
...
describe('Generated Classes', () => {
  ...
  it('Stale Memory', async () => {
    ...
    const acc3 = (
      await Account.retrieve(
        `SELECT Name, Website FROM Account WHERE Id = '${acc.id}'`
      )
    )[0];
  ...
  it('prepareFor Update all', async () => {
    ...
    const acc2 = (
      await Account.retrieve(
        `SELECT Id, Name, CreatedDate FROM Account WHERE Id = '${acc.id}'`
      )
    )[0];
  ...
  it('multi-picklists', async () => {
    ...
    const acc2 = (
      await Account.retrieve(
        `SELECT Id, MultiPick__c, CreatedDate FROM Account WHERE Id = '${acc.id}'`
      )
    )[0];
  ...
  it('refresh', async () => {
    ...
    const acc2 = (
      await Account.retrieve(
        `SELECT Id, Name FROM Account WHERE Id = '${acc.id}'`
      )
    )[0];
  ...
  it('query readonly fields', async () => {
    const acc = new Account({ name: 'testing' });
    await acc.insert();
  ...
  it('should set relation by external id', async () => {
    // setup account
    const accs = await Account.retrieve(f => {
      return {
        select: [...f.select('id', 'testExternalId')],
        where: [{ field: f.select('testExternalId'), op: '=', val: '123abc' }],
      };
    });
    let acc: Account;
    if (accs.length) {
    ...
    const retCont = await Contact.retrieve(f => {
      return {
  ...
  it('composite retrieve', async () => {
    const results = await compositeRetrieve(Contact, Account, User)(
      f => ({ select: f.select('cleanStatus', 'email', 'accountId') }),
      f => ({
        select: f.select('numberOfEmployees', 'accountNumber', 'active'),
      }),
      'select x from blah'
    );
