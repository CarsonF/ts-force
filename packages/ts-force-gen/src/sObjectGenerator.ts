...
export class SObjectGenerator {
  private readonly sObjectConfig: SObjectConfig;
  private readonly allConfigsMap: Map<string, SObjectConfig>;
  private readonly dependsOn: Set<string>;

  private readonly sourceFile: SourceFile;
  private readonly singleFileMode: boolean;

  private readonly client;
  private readonly fieldsTypeAlias: string;

  private readonly pickLists: Map<string, Array<[string, string]>>;
  /**
  ...
  constructor(
    ...
    sObjectConfig: SObjectConfig,
    allConfigs: SObjectConfig[]
  ) {
    this.sObjectConfig = sObjectConfig;
    this.pickLists = new Map<string, Array<[string, string]>>();
  ...
  async generateFile() {
    ...
    if (!this.singleFileMode) {
      ...
      this.sourceFile.addImportDeclaration({
        moduleSpecifier: './',
        namedImports: [...this.dependsOn]
          .filter(c => c !== this.sObjectConfig.className)
          .map(c => {
            return { name: c };
          }),
      });
  ...
  // class generation
  async generateSObjectClass(sobConfig: SObjectConfig): Promise<void> {
    let sobDescribe: SObjectDescribe;
    try {
      sobDescribe = await this.retrieveDescribe(sobConfig.apiName);
    } catch (e) {
      throw new Error(
        `Could not retrieve describe metadata for ${sobConfig.apiName}. Check SObject spelling and authorization `
      );
    }

    const props: PropertyDeclarationStructure[] = [];

    // generate props from fields & children
    props.push(
      ...this.generateChildrenProps(sobConfig, sobDescribe.childRelationships)
    );
    props.push(...this.generateFieldProps(sobConfig, sobDescribe.fields));

    const className = this.sObjectConfig.className;

    const classDeclaration = this.generateClass(sobConfig, className, props);
    ...
    classDeclaration.addMethod({
      ...
      parameters: [
        {
          name: 'qryParam',
          type: `((fields: FieldResolver<${className}>) => SOQLQueryParams) | string`,
        },
    ...
    classDeclaration.addMethod({
      ...
      isStatic: true,
      scope: Scope.Public,
      parameters: [{ name: 'sob', type: 'SObject' }],
  ...
  private generateClass(
    sobConfig: SObjectConfig,
    className: string,
    props: PropertyDeclarationStructure[]
  ): ClassDeclaration {
    ...
    const propsInit = props
      .map(prop => {
        return `this.${prop.name} = void 0;`;
      })
      .join('\n');

    const constructorBody = `super('${sobConfig.apiName}', restInstance);
                            ${propsInit}
  ...
  private generatePickistConst(classDeclaration: ClassDeclaration) {
    if (this.pickLists.size) {
      const picklistsConst = classDeclaration.addProperty({
        name: 'PICKLIST',
        scope: Scope.Public,
        isStatic: true,
        isReadonly: true,
      ...
      const picklistConsts = {};

      this.pickLists.forEach((values, field) => {
        picklistConsts[field] = WriterFunctions.object(
          values.reduce((obj, pv) => {
  ...
  private generateChildrenProps(
    ...
    children.forEach(child => {
      const relatedSobConfig = this.allConfigsMap.get(
        child.childSObject.toLowerCase()
      );
      ...
      const referenceClass = relatedSobConfig.className;

      const decoratorProps: SalesforceDecoratorProps = {
      ...
      let propName = this.sanitizeProperty(
        sobConfig,
      ...
      props.push({
        ...
        type: `${referenceClass}[]`,
        scope: Scope.Public,
        decorators: [this.generateDecorator(decoratorProps)],
      });
    });
    return props;
  }
    ...
    fields.forEach(field => {
      ...
      if (this.sObjectConfig.generatePicklists) {
        if (field.picklistValues.length) {
          ...
          const values = field.picklistValues.map<[string, string]>(pv => {
            const name = this.sanatizePicklistName(pv.value, usedNames);
            usedNames.add(name);
          ...
          if (
            this.sObjectConfig.enforcePicklistValues &&
            ((this.sObjectConfig.enforcePicklistValues === 'RESTRICTED' &&
              field.restrictedPicklist) ||
              this.sObjectConfig.enforcePicklistValues === 'ALL')
          ) {
            const picklist = `PicklistConst<typeof ${sobConfig.className}.PICKLIST.${prop.name}>`;
  ...
  private getDecorator(field: Field): DecoratorStructure {
    const decoratorProps = {
      ...
      required:
        (field.createable || field.updateable) && field.nillable === false,
  ...
  private generateDecorator(
    decoratorProps: SalesforceDecoratorProps
  ): DecoratorStructure {
    const ref =
      decoratorProps.reference != null
        ? `()=>{return ${decoratorProps.reference}}`
        : 'undefined';
    const sfType = decoratorProps.salesforceType
      ? `${this.mapTypeToEnum(decoratorProps.salesforceType)}`
      : 'undefined';
    const label = decoratorProps.salesforceLabel
      ? decoratorProps.salesforceLabel.replace(/'/g, "\\'")
      : '';

    const props: {
      [P in keyof Pick<
        SFieldProperties,
        Exclude<keyof SFieldProperties, 'toString'>
      >]: string;
    } = {
